...
class UnitID:
    ...
    atom_name: Optional[str] = ""  # 0-4 characters, case-insensitive, blank means all atoms
...
def process_alignment(alignment: Dict) -> Dict[str, List[UnitID]]:
    ...
    for key, unit_id_list in alignment.items():
        if not isinstance(unit_id_list, list) or len(unit_id_list) != 6:
            print(f"Warning: Expected list of 6 elements for key '{key}', got {len(unit_id_list) if isinstance(unit_id_list, list) else 'non-list'}")
            continue
