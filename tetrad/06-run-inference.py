#! /usr/bin/env python3
# -*- coding: utf-8 -*-
"""Step 06 — run a trained tetrad model on an arbitrary structure.

Given a structure file (``.cif`` / ``.cif.gz`` / ``.pdb``) and a bundled model
pickle produced by ``05-cluster-and-train.ipynb``, this script:

1. Parses the structure and collects every **guanine** nucleotide (RNA ``G``
   or DNA ``DG``) with a C1' atom.
2. Builds a ``cKDTree`` over the guanine C1' coordinates and enumerates all
   4-tuples within a radius, pruning by max pairwise distance and coplanarity
   RMSD (so that only tetrad-shaped candidates reach the classifier).
3. Canonicalises each candidate's order (same ``canonicalize`` used for
   positives and negatives), computes the 16 geometric features, applies the
...
span multiple strands) but by KD-tree enumeration over guanines.

Usage
-----
    uv run python 06-run-inference.py <structure.cif> --model <bundle.pkl>
    uv run python 06-run-inference.py <structure.cif.gz> --model <bundle.pkl> \\
        --output-csv out.csv --output-plot out.png
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--model", type=Path, required=True, help="Path to a bundled .pkl model")
    p.add_argument("--output-csv", type=Path, help="Output CSV path (default: derived)")
    p.add_argument("--output-plot", type=Path, help="Optional per-candidate probability PNG")
    p.add_argument("--structure-model", type=int, help="1-based model number for multi-model files")
    p.add_argument("--radius", type=float, default=DEFAULT_RADIUS, help="KD-tree neighbourhood radius (Å)")
    p.add_argument("--max-distance", type=float, default=DEFAULT_MAX_DISTANCE, help="Max pairwise C1' distance (Å)")
    p.add_argument("--coplanarity-rmsd", type=float, default=DEFAULT_COPLANARITY_RMSD, help="Max RMSD to best-fit plane (Å)")
    return p.parse_args()
...
def load_model_bundle(path: Path) -> dict[str, Any]:
    ...
    if n != 4:
        raise ValueError(f"{path} window_size={n}, this script expects 4.")
...
def parse_structure_file(path: Path) -> pd.DataFrame:
    ...
    if suffix == ".pdb":
        with open(path) as h:
            return parse_pdb_atoms(h)
    raise ValueError(f"Unsupported structure format '{suffix}'. Expected .cif, .cif.gz or .pdb.")
...
def main() -> None:
    ...
    candidates = enumerate_tuples(
        ...
        coplanarity_rmsd=args.coplanarity_rmsd,
        dedup=True,
    )

    if not candidates:
        raise ValueError(f"No tetrad-shaped candidate {n}-tuples found (try relaxing --radius / --coplanarity-rmsd).")
    ...
    # Drop raw angle columns not present in the training feature set
