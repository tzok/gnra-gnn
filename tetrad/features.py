...
def calculate_geometric_features(coords: Sequence[Point], n: int) -> Dict[str, float]:
    ...
    The number of points must equal ``n``.  Column naming follows the GNRA
    convention: ``d{ij}`` (distance), ``a{ijk}`` + ``as{ijk}`` + ``aa{ijk}``
    (raw / sin / cos planar angle at ``j``), ``t{ijkl}`` + ``ts{ijkl}`` +
    ``ta{ijkl}`` (raw / sin / cos torsion).
    """
    pts = list(coords)
    if len(pts) != n:
    ...
    for i, j in combinations(range(n), 2):
        result[f"d{i}{j}"] = calculate_distance(pts[i], pts[j])

    for i, j, k in combinations(range(n), 3):
        angle, sin_a, cos_a = calculate_planar_angle(pts[i], pts[j], pts[k])
        result[f"a{i}{j}{k}"] = angle
        result[f"as{i}{j}{k}"] = sin_a
        result[f"aa{i}{j}{k}"] = cos_a

    for i, j, k, l in combinations(range(n), 4):
        torsion, sin_t, cos_t = calculate_torsion_angle(pts[i], pts[j], pts[k], pts[l])
        result[f"t{i}{j}{k}{l}"] = torsion
        result[f"ts{i}{j}{k}{l}"] = sin_t
        result[f"ta{i}{j}{k}{l}"] = cos_t
