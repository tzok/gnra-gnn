#! /usr/bin/env python3
# -*- coding: utf-8 -*-
"""Step 02 — extract positive examples (G-tetrads) as individual CIF files.

Reads ``tetrad_motifs_by_pdb.json`` (produced by step 01), opens each
corresponding assembly mmCIF from the OnQuadro mirror, locates the four
guanine residues of every annotated tetrad, canonicalises their order via
``canonical_order.canonicalize`` and writes a single CIF file per tetrad into
``tetrad/motif_cif_files/``.

Key differences from the GNRA ``02-generate-positive.py``:

* No consecutive-residue / single-chain checks (a tetrad's four Gs come from
  up to four different strands).
* No ±1 extension — the motif *is* the four residues.
* The residues are reordered to the canonical geometric order before being
  written, so that downstream featurisation and nRMSD clustering see a
  consistent ordering.
"""
...
def find_residues(
    residues: List[Residue], unit_ids: List[Dict[str, Any]]
) -> Optional[List[Residue]]:
    """Match each unit_id to a residue by (chain_id, residue_number, icode)."""
    found: List[Residue] = []
    for uid in unit_ids:
...
def c1_prime_coords(residues: List[Residue]) -> Optional[List[Tuple[float, float, float]]]:
    """Extract C1' coordinates from each residue; ``None`` if any is missing."""
...
def process_one_stem(
    ...
    """Process all motifs for one (pdb, assembly) stem.
    ...
    for motif in motifs:
        ...
        if len(unit_ids) != TETRAD_SIZE:
            print(f"    Warning: {motif_key} has {len(unit_ids)} unit_ids, expected {TETRAD_SIZE}")
            n_skipped += 1
            continue

        matched = find_residues(residues, unit_ids)
        if matched is None:
            print(f"    Warning: {motif_key} — could not locate all 4 residues")
            n_skipped += 1
            continue

        coords = c1_prime_coords(matched)
        if coords is None:
            print(f"    Warning: {motif_key} — missing C1' for at least one residue")
            n_skipped += 1
            continue
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Path to tetrad_motifs_by_pdb.json",
    )
    parser.add_argument(
        "--mirror",
    ...
    parser.add_argument(
        ...
        help="Output directory for per-tetrad CIF files",
    )
    ...
    print(f"Loaded {total_motifs} G4 tetrads from {len(motifs_by_pdb)} files")

    # Filter out stems where all motifs are already processed
    ...
    print(f"Will process {len(pending)} stems with {sum(len(v) for v in pending.values())} tetrads")
    ...
    print(f"\nDone. Saved {total_saved} tetrads, skipped {total_skipped}.")
