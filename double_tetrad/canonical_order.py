...
3. Project the centred points onto the plane and compute their polar angle
   via ``atan2``.  Sorting these angles gives a cyclic order.  When two points
   share the same angle (e.g. two G-tetrads stacked without twist, so that the
   four guanines of the upper tetrad project directly onto the four of the
   lower), the **height along the normal** is used as a secondary sort key,
   guaranteeing a deterministic order even in that degenerate case.
...
inference candidates (step 06), guaranteeing that a given geometry always
produces the same feature vector.
"""
...
def canonicalize(coords: Sequence[Point]) -> Tuple[List[int], List[Point]]:
    ...
    # Height along the normal — used as a secondary sort key when two points
    # share the same planar angle (degenerate case: stacked tetrads without
    # twist).  lexsort sorts by last key first, so (angles, heights) means
    # "sort by angle, break ties by height".
    heights = centred @ normal

    # Cyclic order by angle, with height as tiebreaker
    cyclic = list(np.lexsort((heights, angles)))
...
def canonical_permutation_key(coords: Sequence[Point]) -> Tuple[Tuple[float, ...], ...]:
    ...
    Two point sets that canonize to the same key are the same geometry up to
